...
def _parse_entry(lines: list[str]) -> tuple[dict[str, Any] | None, list[dict[str, Any]]]:
    accession = ""
    full_name = ""
    gene_name = ""
    go_edges: list[dict[str, Any]] = []

    for line in lines:
        if line.startswith("AC   ") and not accession:
            payload = line[5:].strip().rstrip(";")
            first = payload.split(";", 1)[0].strip()
            accession = first
        elif line.startswith("DE   RecName: Full=") and not full_name:
            full_name = line.split("Full=", 1)[1].rstrip(";").strip()
            ...
            if len(parts) < 3:
                continue
            go_id = parts[1]
            aspect = parts[2][:1] if parts[2] else ""
            ...
            go_edges.append({"go_id": go_id, "predicate": predicate, "go_category": category})
    ...
    node = {
        ...
        "name": full_name or gene_name or accession,
        "category": "biolink:Protein",
        "provided_by": "uniprot",
        "source_payload": json.dumps({"gene_name": gene_name}, default=str),
    }
    return node, go_edges


def _parse_uniprot(path: Path, entry_limit: int) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    nodes: dict[str, dict[str, Any]] = {}
    edges: dict[str, dict[str, Any]] = {}

    with _open_dat(path) as handle:
        current: list[str] = []
        count = 0
        for raw in handle:
            line = raw.rstrip("\n")
            if line == "//":
                node, go_edges = _parse_entry(current)
                current = []
                if node is None:
                    continue
                nodes[node["id"]] = node
                for ge in go_edges:
                    ...
                    if go_id not in nodes:
                        nodes[go_id] = {
                            "id": go_id,
                            "name": go_id,
                            "category": ge["go_category"],
                            "provided_by": "uniprot",
                            "source_payload": json.dumps({}, default=str),
                        }
                    eid = _edge_id(node["id"], ge["predicate"], go_id, "uniprot_go")
                    edges[eid] = {
                        "id": eid,
                        "subject": node["id"],
                        "object": go_id,
                        ...
                        "provided_by": "uniprot",
                        "source_payload": json.dumps(ge, default=str),
                    }
                count += 1
                if entry_limit > 0 and count >= entry_limit:
                    break
            else:
                current.append(line)
...
def _upsert_nodes(connection: dict[str, Any], nodes: list[dict[str, Any]], batch_size: int) -> int:
    ...
    for chunk in _batch(nodes, batch_size):
        ...
        _validate_no_errors(payload, "upsert_uniprot_nodes")
        total += len(chunk)
    return total
...
def _upsert_edges(connection: dict[str, Any], edges: list[dict[str, Any]], batch_size: int) -> int:
    ...
    for chunk in _batch(edges, batch_size):
        ...
        _validate_no_errors(payload, "upsert_uniprot_edges")
        total += len(chunk)
    return total


def main() -> None:
    parser = argparse.ArgumentParser(description="Convert UniProt DAT entries into Nexus KG protein and GO edges.")
    parser.add_argument("--config", default="configs/kg_build/uniprot_dat_to_nexus.yaml", help="YAML config path")
    ...
    source_cfg = cfg.get("source_uniprot", {}) if isinstance(cfg, dict) else {}
    ...
    if not isinstance(source_cfg, dict) or not isinstance(nexus_cfg, dict):
        raise ValueError("source_uniprot and nexus_connection sections are required and must be mappings.")

    dat_path = Path(str(source_cfg.get("path", "")).strip())
    if not dat_path.is_absolute():
        dat_path = PROJECT_ROOT / dat_path
    if not dat_path.exists():
        raise FileNotFoundError(f"UniProt DAT file not found: {dat_path}")

    entry_limit = int(source_cfg.get("entry_limit", 5000))
    ...
    nodes, edges = _parse_uniprot(dat_path, entry_limit=entry_limit)
